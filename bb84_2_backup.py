...
# ============================================================
# LEGACY: BB84Simulator class definition (for compatibility)
# NOTE: This is now imported from bb84_simulator.py
# Keeping this reference for backward compatibility only
# ============================================================
class BB84Simulator:
    def __init__(self):
    ...
    def simulate_transmission(self, alice_bits, alice_bases, bob_bases, eve_present=False, eve_intercept_prob=0.5, noise_prob=0.0):
        alice_bits = np.array([int(b) for b in list(alice_bits)])
        alice_bases = np.array([int(b) for b in list(alice_bases)])
        bob_bases = np.array([int(b) for b in list(bob_bases)])
        
        n = len(alice_bits)
        bob_results = np.zeros(n, dtype=int)
        eve_results = np.zeros(n, dtype=int) if eve_present else None
        eve_bases = None
        ...
        # Batch processing for speed: process multiple qubits per job
        batch_size = 20  # Process 20 qubits at a time for optimal performance
        ...
        for batch_start in range(0, n, batch_size):
            ...
            for i in batch_indices:
                qc = self.encode_qubit(alice_bits[i], alice_bases[i])
    ...
    def privacy_amplification(sifted_key, error_rate, target_security_level=1e-6):
        sifted_key = np.array([int(b) for b in list(sifted_key)])
        n = len(sifted_key)
        if n == 0:
            return []
...
def create_transmission_timeline(alice_bits, alice_bases, bob_bases, bob_results, title="BB84 Transmission Timeline"):
    """Create detailed timeline visualization of BB84 transmission - Optimized"""
    ...
    errors = (alice_bits != bob_results) & matches  # Errors only on matching bases
    
    # Create timeline data using vectorized operations
    n = len(alice_bits)
    timeline_data = {
        'BitIndex': np.arange(n),
        'AliceBit': alice_bits,
        ...
        'Used': matches,
        'Error': errors,
        'ErrorType': np.where(~matches, 'Basis Mismatch', np.where(errors, 'Transmission Error', 'None'))
    }
...
def main():
    ...
    # Track last parameter values to detect if simulation needs to run
    if "last_num_bits" not in st.session_state:
        st.session_state.last_num_bits = None
    
    # Simulation results storage to prevent recalculation
    ...
    **BB84** is the first quantum key distribution protocol, proposed by Charles Bennett and Gilles Brassard in 1984. It allows two parties, Alice and Bob, to securely share a secret key over an insecure channel, with security guaranteed by quantum mechanics.
    ...
    if run_button:
        # Always run fresh simulation with current parameters
        ...
        st.session_state.sim_results = None
        st.session_state.last_num_bits = num_bits  # Update last used value
    ...
    if st.session_state.simulation_run and not st.session_state.simulation_completed:
        ...
        with st.spinner("🔬 **Running BB84 Quantum Simulation...**"):
            ...
            # Store bits in session state for consistency
            ...
            # Store all results in session state to prevent recalculation and keep data consistent
            ...
            # Use local variables (just computed in compute() function above)
            # Note: We'll use the actual computed values before storing them
            
            steps = [
                ("**Step 1: Alice Generates Random Bits**", "Alice creates random bits and bases for secure communication.", str(st.session_state.alice_bits_stored[:20]) + "..."),
                ("**Step 2: Alice Prepares Qubits**", "Alice encodes bits into quantum states using chosen bases.", str([state_label(b, a) for b, a in zip(st.session_state.alice_bits_stored[:20], st.session_state.alice_bases_stored[:20])]) + "..."),
                ("**Step 3: Qubits Sent to Bob**", "Qubits are transmitted through the quantum channel.", "Transmission in progress..."),
                ("**Step 4: Bob Measures Qubits**", "Bob measures qubits in his randomly chosen bases.", str(st.session_state.sim_results['bob_no_eve'][:20]) + "..."),
                ("**Step 5: Basis Announcement**", "Alice and Bob publicly announce their bases (not the bits).", "Bases compared..."),
                ("**Step 6: Sifting**", "Only bits with matching bases are kept (sifted key).", str(st.session_state.sim_results['tl_no_eve'][st.session_state.sim_results['tl_no_eve']["Used"] == True]["AliceBit"].tolist()[:20]) + "..."),
                ("**Step 7: Error Estimation**", f"QBER calculated to detect eavesdropping: {st.session_state.sim_results['qber_no']:.3f}", "Checking for eavesdropping..."),
                ("**Step 8: Privacy Amplification**", "Hashing reduces key length for security.", "Key generated securely." if st.session_state.sim_results['key_no'] > 0 else "No secure key due to errors.")
            ]
            ...
            # Extract stored results for display (only if simulation was successful)
            if st.session_state.sim_results is not None:
                    ...
                    with action_col1:
                        if st.button("**Detailed Metrics**", help="Show comprehensive performance metrics", key="btn_detailed"):
                            st.session_state.show_detailed = not st.session_state.get('show_detailed', False)
                    with action_col2:
                        if st.button("**Error Analysis**", help="Analyze error patterns in detail", key="btn_errors"):
                            st.session_state.show_errors = not st.session_state.get('show_errors', False)
                    with action_col3:
                        if st.button("**Key Statistics**", help="Display key generation statistics", key="btn_stats"):
                            st.session_state.show_stats = not st.session_state.get('show_stats', False)
                    with action_col4:
                        if st.button("**Parameter Summary**", help="Show current simulation parameters", key="btn_params"):
                    ...
                    with tab4:
                        ...
                        # Create sub-tabs for different visualization modes
                    ...
                    with tab5:
                        ...
                        with dl_col3:
                            # Lazy-load PDF generation - only create on demand
                            @st.cache_data
                            def get_pdf_bytes():
                                # Convert to cacheable formats (tuples and strings)
                    ...
                    # Show balloons after ALL visualizations and content are loaded
